...
export default async function MyListingsPage({ searchParams }: { searchParams: { success?: string } }) {
  ...
  return (
    <div className="max-w-[820px] mx-auto px-5 sm:px-6 py-10 pb-20">
      <Link href="/cho-do-cu" className="inline-flex items-center gap-1.5 text-[12.5px] font-medium text-muted hover:text-rose transition-colors mb-6">
        ← {t('back_to_market')}
      </Link>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap mb-7">
        ...
        <Link href="/cho-do-cu/dang" className="inline-flex items-center gap-2 font-semibold text-[13.5px] px-5 py-2.5 rounded-full bg-rose text-white hover:bg-rose-deep transition-all">＋ {t('post_listing')}</Link>
      </div>
      ...
      {listings.length === 0 ? (
        <div className="bg-paper border border-line rounded-2xl px-6 py-14 text-center">
          ...
          <Link href="/cho-do-cu/dang" className="inline-flex items-center gap-2 font-semibold text-[13.5px] px-6 py-2.5 rounded-full bg-rose text-white hover:bg-rose-deep transition-all">＋ {t('post_listing')}</Link>
        </div>
      ) : (
        <div className="space-y-3">
          {listings.map((l) => (
            <div key={l.id} className="flex gap-4 bg-paper border border-line rounded-2xl p-3.5">
              <Link href={`/cho-do-cu/${l.id}`} className="flex-none w-20 h-20 rounded-xl overflow-hidden bg-cream">
                {l.cover_image ? (
              ...
              <div className="flex-1 min-w-0">
                ...
                <Link href={`/cho-do-cu/${l.id}`} className="font-semibold text-[15px] text-ink leading-snug line-clamp-1 hover:text-rose transition-colors">{l.title}</Link>
              ...
              <div className="flex-none self-center">
                <Link href={`/cho-do-cu/sua/${l.id}`} className="text-[12.5px] font-semibold px-3.5 py-2 rounded-full bg-cream border border-line text-ink hover:border-rose/30 transition-all">{t('edit')}</Link>
              </div>
            </div>
          ))}
        </div>
