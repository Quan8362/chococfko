...
export default async function EditListingPage({ params }: { params: { id: string } }) {
  ...
  if (!listing) notFound()
  if (listing.user_id !== user.id) redirect(`/cho-do-cu/${params.id}`)

  return (
    <div className="max-w-[760px] mx-auto px-5 sm:px-6 py-10 pb-20">
      <Link href="/cho-do-cu/cua-toi" className="inline-flex items-center gap-1.5 text-[12.5px] font-medium text-muted hover:text-rose transition-colors mb-6">
        ← {t('my_listings')}
      </Link>
