...
import { fetchWaitingChessRooms, joinChessRoomFromLobby, type WaitingChessRoom } from './actions'

type Props = {
  initialRooms: WaitingChessRoom[]
  userId: string | null
}
...
export default function ChineseChessWaitingRooms({ initialRooms, userId }: Props) {
  const t = useTranslations('games.chinese_chess')
  const router = useRouter()
  const [rooms, setRooms] = useState<WaitingChessRoom[]>(initialRooms)
  const [error, setError] = useState<string | null>(null)
  const [joiningRoom, setJoiningRoom] = useState<string | null>(null)
  const [isPending, startTransition] = useTransition()

  // Realtime: refresh on any chinese_chess_rooms change
  useEffect(() => {
    const supabase = createClient()
    const channel = supabase
      .channel('chess_lobby')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'chinese_chess_rooms' }, async () => {
        const updated = await fetchWaitingChessRooms()
        setRooms(updated)
      })
      .subscribe()
  ...
  // Polling fallback: catch stale rooms that didn't trigger DB events
  useEffect(() => {
  ...
  const handleJoin = (roomCode: string) => {
    ...
    startTransition(async () => {
      const result = await joinChessRoomFromLobby(roomCode)
      if (result?.error) {
        const msg =
        ...
        router.push(`/games/chinese-chess/${roomCode}`)
      }
    })
  }
      ...
      {rooms.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {rooms.map(room => (
            ...
            >
              {/* Red piece indicator */}
              <div className="w-9 h-9 rounded-xl bg-red-50 border border-red-200/70 flex items-center justify-center flex-none">
                <span className="font-black text-[18px] text-red-600 leading-none font-serif">帥</span>
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                  ...
                  <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200 flex-none">
                    {t('lobby_side_red')}
                  </span>
                </div>
                <p className="text-[12.5px] text-muted/70 truncate">
                  <span className="text-muted/50">{t('lobby_host')}: </span>
                  <span className="font-medium">{room.player_red_name}</span>
                </p>
