...
export async function generateMetadata({ params }: { params: { roomCode: string } }) {
  return { title: `Phòng ${params.roomCode.toUpperCase()} · Cờ Tướng · Chợ Cóc FKO` }
}
...
export default async function ChineseChessRoomPage({ params }: { params: { roomCode: string } }) {
  ...
  // Fetch player names + initial move history in parallel
  const [playerRedName, playerBlackName, { data: movesData }] = await Promise.all([
    getPlayerName(admin, finalRoom.player_red),
    finalRoom.player_black ? getPlayerName(admin, finalRoom.player_black) : Promise.resolve(null),
    admin
