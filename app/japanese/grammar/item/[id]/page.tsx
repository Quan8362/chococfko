...
export async function generateMetadata({ params }: { params: { id: string } }) {
  ...
  if (!g) return { title: t('grammar_heading'), robots: { index: false, follow: true } }
  const canonical = `https://chococfko.com/japanese/grammar/item/${params.id}`
  const description = g.meaning_vi ?? g.meaning_en ?? undefined
  const title = `${g.pattern} · ${g.jlpt_level ?? ''} ${t('grammar_heading')}`.replace(/\s+/g, ' ').trim()
  return {
    title,
    description,
    alternates: { canonical },
    openGraph: { type: 'article', url: canonical, title: `${title} · Chợ Cóc FKO`, description },
    twitter: { card: 'summary_large_image', title: `${title} · Chợ Cóc FKO`, description },
  }
}
...
export default async function GrammarDetailPage({ params }: { params: { id: string } }) {
  ...
  const currentUser = user
    ...
    : null
  const pagePath = `/japanese/grammar/item/${grammar.id}`
  const pageUrl = `https://chococfko.com${pagePath}`
    ...
    '@graph': [
      {
        ...
        description: meaning ?? undefined,
        url: pageUrl,
        inLanguage: 'ja',
        educationalLevel: grammar.jlpt_level ? `JLPT ${grammar.jlpt_level}` : undefined,
        inDefinedTermSet: {
          '@type': 'DefinedTermSet',
          '@id': 'https://chococfko.com/japanese/grammar#set',
          name: t('grammar_heading'),
          url: 'https://chococfko.com/japanese/grammar',
        },
      },
      {
        '@type': 'BreadcrumbList',
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: t('page_heading'), item: 'https://chococfko.com/japanese' },
          { '@type': 'ListItem', position: 2, name: t('grammar_heading'), item: 'https://chococfko.com/japanese/grammar' },
          { '@type': 'ListItem', position: 3, name: grammar.pattern, item: pageUrl },
        ],
      },
    ],
  }
  ...
  return (
    <div className="max-w-[860px] mx-auto px-5 sm:px-6 py-10 pb-20">
      ...
      <nav className="flex items-center gap-1.5 text-[12.5px] text-muted mb-7 flex-wrap">
        ...
        <span>/</span>
        <Link href="/japanese/grammar" className="hover:text-rose transition-colors">{t('grammar_heading')}</Link>
        {grammar.jlpt_level && dbLevel(levelUrl) && (
          <>
            <span>/</span>
            <Link href={`/japanese/grammar/${levelUrl}`} className="hover:text-rose transition-colors">
              {grammar.jlpt_level}
            </Link>
          </>
        )}
