...
export async function generateMetadata({ params }: { params: { level: string } }) {
  ...
  return {
    title: `${level} ${t('kanji_heading')} · ${t('page_heading')}`,
    alternates: { canonical: `https://chococfko.com/japanese/kanji/${urlLevel(level)}` },
  }
}
  ...
  return (
    <div className="max-w-[1080px] mx-auto px-5 sm:px-6 py-10 pb-20">
      ...
      <nav className="flex items-center gap-1.5 text-[12.5px] text-muted mb-8 flex-wrap">
        ...
        <span>/</span>
        <Link href="/japanese/kanji" className="hover:text-rose transition-colors">
          {t('kanji_heading')}
        </Link>
        <span>/</span>
      ...
      <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {JLPT_LEVELS.map(l => (
            <Link
              key={l}
              href={`/japanese/kanji/${urlLevel(l)}`}
