...
export async function generateMetadata() {
  ...
  return {
    title: `${t('kanji_heading')} · ${t('page_heading')}`,
    alternates: { canonical: 'https://chococfko.com/japanese/kanji' },
  }
}
...
export default async function KanjiHubPage() {
  ...
  const levels = JLPT_LEVELS.map(level => ({
    ...
    href:       `/japanese/kanji/${urlLevel(level)}`,
    label:      t('study_kanji'),
    countLabel: 'kanji',
  }))
  ...
  return (
    <div className="max-w-[960px] mx-auto px-5 sm:px-6 py-10 pb-20">
      ...
      <nav className="flex items-center gap-1.5 text-[12.5px] text-muted mb-8">
        ...
        <span>/</span>
        <span className="text-ink">{t('kanji_heading')}</span>
      </nav>
      ...
      <div className="mb-10">
        ...
        <p className="text-[14px] text-muted">{t('kanji_desc')}</p>
      </div>
