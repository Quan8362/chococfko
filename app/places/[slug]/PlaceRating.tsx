...
import { submitPlaceRating } from '../actions'

export default function PlaceRating({
  slug, average, count, myStars, myReview, isLoggedIn,
}: {
  slug: string
  average: number
  count: number
  ...
  const t = useTranslations('place_rating')
  ...
  async function save() {
    ...
    const res = await submitPlaceRating(slug, stars, review)
    setSaving(false)
