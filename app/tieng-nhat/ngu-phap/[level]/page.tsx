...
export async function generateMetadata({ params }: { params: { level: string } }) {
  ...
  return { title: `${level} ${t('grammar_heading')} · ${t('page_heading')} · Chợ Cóc FKO` }
}
  ...
  return (
    <div className="max-w-[1080px] mx-auto px-5 sm:px-6 py-10 pb-20">
      ...
      <nav className="flex items-center gap-1.5 text-[12.5px] text-muted mb-8 flex-wrap">
        ...
        <span>/</span>
        <Link href="/tieng-nhat/ngu-phap" className="hover:text-rose transition-colors">
          {t('grammar_heading')}
        </Link>
        <span>/</span>
      ...
      <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {JLPT_LEVELS.map(l => (
            <Link
              key={l}
              href={`/tieng-nhat/ngu-phap/${urlLevel(l)}`}
