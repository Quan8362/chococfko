  ...
  return (
    <div className="flex flex-wrap items-center gap-x-0.5 gap-y-1 px-2.5 py-2 border-b border-line bg-cream/60">
      ...
      <div className="flex items-center gap-0.5 flex-none">
        <Btn onClick={() => e.chain().focus().toggleBold().run()} active={e.isActive('bold')} title="Bold (Ctrl+B)">
          <b className="font-black text-[14px]">B</b>
        </Btn>
        <Btn onClick={() => e.chain().focus().toggleItalic().run()} active={e.isActive('italic')} title="Italic (Ctrl+I)">
      ...
      {/* Headings */}
      <div className="flex items-center gap-0.5 flex-none">
        <Btn onClick={() => e.chain().focus().setParagraph().run()} active={e.isActive('paragraph')} title="Đoạn văn">
          <span className="text-[11px] font-semibold">¶</span>
        </Btn>
        <Btn onClick={() => e.chain().focus().toggleHeading({ level: 2 }).run()} active={e.isActive('heading', { level: 2 })} title="Tiêu đề lớn (H2)">
          <span className="text-[11.5px] font-bold">H2</span>
        </Btn>
        <Btn onClick={() => e.chain().focus().toggleHeading({ level: 3 }).run()} active={e.isActive('heading', { level: 3 })} title="Tiêu đề nhỏ (H3)">
      ...
      {/* Lists */}
      <div className="flex items-center gap-0.5 flex-none">
        <Btn onClick={() => e.chain().focus().toggleBulletList().run()} active={e.isActive('bulletList')} title="Danh sách chấm">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        </Btn>
        <Btn onClick={() => e.chain().focus().toggleOrderedList().run()} active={e.isActive('orderedList')} title="Danh sách số">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" />
          </svg>
        </Btn>
        <Btn onClick={() => e.chain().focus().toggleBlockquote().run()} active={e.isActive('blockquote')} title="Trích dẫn">
      ...
      {/* Alignment */}
      <div className="flex items-center gap-0.5 flex-none">
        <Btn onClick={() => e.chain().focus().setTextAlign('left').run()} active={e.isActive({ textAlign: 'left' })} title="Căn trái">
        ...
        <Btn onClick={() => e.chain().focus().setTextAlign('center').run()} active={e.isActive({ textAlign: 'center' })} title="Căn giữa">
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
        ...
        <Btn onClick={() => e.chain().focus().setTextAlign('right').run()} active={e.isActive({ textAlign: 'right' })} title="Căn phải">
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="flex items-center gap-0.5 flex-none">
        <Btn
          onClick={() => {
            const url = prompt('Nhập URL (vd: https://...):')
        ...
        <Btn onClick={onImageUpload} title="Chèn ảnh vào bài">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
        </Btn>
        <Btn onClick={() => e.chain().focus().setHorizontalRule().run()} title="Đường kẻ ngang">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Callout blocks */}
      <div className="flex items-center gap-0.5 flex-none">
        <span className="text-[10px] text-muted/70 font-medium mr-0.5 hidden sm:block">Block:</span>
        {CALLOUTS.map((c) => (
          <Btn
            key={c.emoji}
      ...
      {/* Undo / Redo */}
      <div className="flex items-center gap-0.5 flex-none">
        <Btn onClick={() => e.chain().focus().undo().run()} disabled={!e.can().undo()} title="Hoàn tác (Ctrl+Z)">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
          </svg>
        </Btn>
        <Btn onClick={() => e.chain().focus().redo().run()} disabled={!e.can().redo()} title="Làm lại (Ctrl+Shift+Z)">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function RichTextEditor({
  name,
  defaultValue = '',
  placeholder = 'Hãy chia sẻ cảm nhận của bạn: địa điểm này có gì hay, nên đi lúc nào, cách đi ra sao, chi phí khoảng bao nhiêu?',
  minHeight = '220px',
}: Props) {
    ...
    <div className="border border-line rounded-xl overflow-hidden bg-white focus-within:border-rose focus-within:ring-2 focus-within:ring-rose/15 transition-all">
      ...
      {pendingDraft && (
        <div className="flex items-center gap-3 px-4 py-2.5 bg-amber-50 border-b border-amber-200 text-[13px]">
          ...
          <span className="text-amber-800 flex-1">Bạn có bản nháp chưa hoàn thành.</span>
          <button
            type="button"
