...
function MatchCard({
  ...
  const Side = ({ name, isWinner, score, present }: { name: string | null; isWinner: boolean; score: number | null; present: boolean }) => (
    <div className={`flex items-center justify-between gap-2 px-2.5 py-1.5 ${isWinner ? 'bg-teal-soft' : ''}`}>
      ...
      {score !== null && <span className={`flex-none text-[12px] font-bold ${isWinner ? 'text-teal' : 'text-muted'}`}>{score}</span>}
    </div>
  )

  return (
    <div className={`rounded-xl border overflow-hidden ${tone === 'third' ? 'border-amber-200' : 'border-line'} bg-paper`}>
      <Side name={a} present={!!match.competitorAId} isWinner={done && match.winnerId === match.competitorAId} score={done ? match.gamesWonA : null} />
      <div className="h-px bg-line" />
      <Side name={b} present={!!match.competitorBId} isWinner={done && match.winnerId === match.competitorBId} score={done ? match.gamesWonB : null} />
