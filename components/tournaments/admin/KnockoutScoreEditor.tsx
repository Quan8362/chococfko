...
export type KnockoutSaveAction = (
  tournamentId: string,
  eventId: string,
  matchId: string,
...
export type KnockoutClearAction = (
  tournamentId: string,
  eventId: string,
  matchId: string,
  expectedMatchVersion: number,
...
// Modal for entering / editing the game scores of ONE knockout match. Mirrors the group ScoreEditor:
// the client does only OPTIMISTIC validation (integer, non-negative, no per-game tie, decisive) — the
// authoritative winner is derived on the server via the pure engine, which also advances the winner
// (and, for a semifinal, the loser) into the next slot and refuses a correction whose downstream is
// already completed.
interface Row {
  a: string
  b: string
...
export default function KnockoutScoreEditor({
  tournamentId,
  eventId,
  match,
  nameA,
  ...
  const te = useTranslations('admin_knockout_results')
  const router = useRouter()
  const [pending, startTransition] = useTransition()
  const [error, setError] = useState<KnockoutMutationError | null>(null)
  const [confirmClear, setConfirmClear] = useState(false)
  // Prompt 11: when a correction is refused because downstream is completed, offer the impact preview.
  const [impact, setImpact] = useState<KnockoutImpactPreview | null>(null)
  ...
  const parsed = useMemo(
    () =>
      rows.map((r) => {
        ...
        const valid = filled && Number.isInteger(a) && Number.isInteger(b) && a >= 0 && b >= 0 && a !== b
  ...
  const currentGames = () => parsed.map((p) => ({ scoreA: p.a, scoreB: p.b }))

  function doSave() {
    setError(null)
    const games = currentGames()
    startTransition(async () => {
      const res = await saveAction(tournamentId, eventId, match.id, match.version, games)
      if (res.ok) {
        onClose()
        router.refresh()
      } else if (res.error === 'downstream_has_results') {
        // The correction changes the winner and a downstream match is already completed → don't just
        // fail: load the read-only impact preview so the admin can review and (with RESET) reset the path.
        const pv = await previewAffectedKnockoutPath(tournamentId, eventId, match.id, games)
        if (pv.ok) setImpact(pv.preview)
  ...
  function doClear() {
    setError(null)
    startTransition(async () => {
      const res = await clearAction(tournamentId, eventId, match.id, match.version)
      if (res.ok) {
        onClose()
        router.refresh()
      } else {
  ...
  return (
    ...
    >
      <div className="w-full max-w-[440px] max-h-[90vh] overflow-y-auto bg-paper border border-line rounded-2xl shadow-xl p-5" onClick={(e) => e.stopPropagation()}>
        ...
        {error && (
          <div className="rounded-lg bg-red-50 border border-red-200 px-3 py-2.5 mb-3" role="alert">
            <p className="text-[13px] text-red-600">{te(`err_${error}`)}</p>
          </div>
        )}
            ...
            return (
              <div key={i} className="flex items-center gap-2">
                ...
                <input
                  ...
                  className={`flex-1 min-w-0 text-center text-[14px] px-2 py-1.5 rounded-lg border bg-cream focus:outline-none focus:border-rose/50 ${invalid ? 'border-red-300' : 'border-line'}`}
                />
        ...
        <button type="button" onClick={addRow} className="mt-2 text-[12.5px] font-semibold text-teal hover:text-teal/80 transition-colors">
          + {t('add_game')}
        </button>
