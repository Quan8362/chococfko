...
CSV_FILE = r"C:\Users\QuanLV17\Downloads\chococfko-web\web\data\japanese\jmdict-n3-vi-ready-022.csv"
...
print(f"Approving {len(source_ids)} N3 entries...")
total = 0
for i in range(0, len(source_ids), 80):
    ...
    req = urllib.request.Request(url, data=json.dumps({"review_status": "approved", "is_published": True}).encode(),
        method="PATCH", headers={"apikey": env["SUPABASE_SERVICE_ROLE_KEY"],
        "Authorization": "Bearer " + env["SUPABASE_SERVICE_ROLE_KEY"],
        "Content-Type": "application/json", "Prefer": "return=representation"})
    with urllib.request.urlopen(req) as r:
        count = len(json.loads(r.read()))
        total += count
        print(f"  chunk {i//80 + 1}: {count} approved")
