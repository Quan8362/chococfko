# -*- coding: utf-8 -*-
"""Approve + publish wave 043 ai_draft rows (matched by source_id, guarded to jmdict ai_draft)."""
import os, csv, urllib.request, json
HERE = os.path.dirname(__file__)
CSV_FILE = os.path.join(HERE, "..", "data", "japanese", "jmdict-n3-vi-ready-043.csv")
...
print(f"Approving up to {len(source_ids)} N3 entries (wave 043)...")
total = 0
for i in range(0, len(source_ids), 80):
    ...
    with urllib.request.urlopen(req) as r:
        count = len(json.loads(r.read())); print(f"  chunk {i//80+1}: approved {count}"); total += count
