-- Migration v2: Community Chat Polls (idempotent — safe to re-run)
...
DROP POLICY IF EXISTS "polls_select_authenticated" ON community_chat_polls;
CREATE POLICY "polls_select_authenticated" ON community_chat_polls
  FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "polls_insert_own" ON community_chat_polls;
CREATE POLICY "polls_insert_own" ON community_chat_polls
  FOR INSERT TO authenticated WITH CHECK (created_by = auth.uid());

-- community_chat_poll_options
DROP POLICY IF EXISTS "poll_options_select_authenticated" ON community_chat_poll_options;
CREATE POLICY "poll_options_select_authenticated" ON community_chat_poll_options
  FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "poll_options_insert" ON community_chat_poll_options;
...
DROP POLICY IF EXISTS "poll_votes_select_authenticated" ON community_chat_poll_votes;
CREATE POLICY "poll_votes_select_authenticated" ON community_chat_poll_votes
  FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "poll_votes_insert_own" ON community_chat_poll_votes;
CREATE POLICY "poll_votes_insert_own" ON community_chat_poll_votes
  FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "poll_votes_delete_own" ON community_chat_poll_votes;
